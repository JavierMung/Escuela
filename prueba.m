...
while choice~=1
    msgbox("Con calma vaquero","Warn","warn");
    choice= menu('¿Que programa deseas correr?','Practica 1','Practica 2');
end
...
answer = inputdlg(prompt,dlg_tilte,num_lines,def);

vector=[str2num(answer{1});str2num(answer{2})];
...
distancia6=norm(media6-vector);
    ...
    waitfor(msgbox(message,"Clase"));
end

s2 = questdlg('¿Deseas buscar otro vector?', ...
	'Yes', ...
	'No');
